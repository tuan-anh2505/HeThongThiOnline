...
export default function StudentPage({ onLogout, onStartExam }) {
  ...
  useEffect(() => {
    getDashboardStudent()
      .then(d => { setData(d); setSelCls(d.classes?.[0]); })
      .catch(() => { setData(MOCK_DASHBOARD_STUDENT); setSelCls(MOCK_DASHBOARD_STUDENT.classes[0]); })
  ...
  const avgDiem  = expired.length
    ? (expired.reduce((s, e) => s + (e.diem || 0), 0) / expired.length).toFixed(1) : "—";
  const maxDiem  = expired.length ? Math.max(...expired.map(e => e.diem || 0)) : "—";
        ...
        {section === "home" && (
          <div>
            ...
            <div style={{ ...L.statRow, marginTop: 24 }}>
              ...
              <StatCard icon="✅" label="Đã hoàn thành"    value={expired.length} color="teal"   />
        ...
        {section === "exams" && (
          <div>
            ...
            <div style={L.statRow}>
              ...
              <StatCard icon="✅" label="Đã làm"         value={expired.length} color="teal"   />
            ...
            <div style={L.toolbar}>
              ...
              <input style={L.search} placeholder="🔍 Tìm bài thi..."
        ...
        {section === "grades" && (
          <div>
            ...
            <div style={{ ...L.statRow, marginTop: 20 }}>
              <StatCard icon="✅" label="Bài đã làm"    value={expired.length} color="teal"   />
              <StatCard icon="⭐" label="Điểm TB"        value={avgDiem}        color="amber"  />
              <StatCard icon="🏆" label="Điểm cao nhất"  value={maxDiem}        color="pink"   />
              <StatCard icon="❌" label="Chưa đạt"
                value={expired.filter(e=>e.diem<5).length} color="red" />
            </div>
            <div style={L.homeCard}>
...
function Loader() {
  return <div style={{display:"flex",alignItems:"center",justifyContent:"center",
    height:"100vh",fontFamily:"'Nunito',sans-serif",fontSize:18,color:"#7F77DD"}}>⏳ Đang tải...</div>;
}
function Empty() {
  return <div style={{textAlign:"center",padding:40,color:"#B4B2A9",fontWeight:700,
    fontSize:15,background:"white",borderRadius:16}}>😔 Không có bài thi nào</div>;
}
...
const L = {
  ...
  tab:      { background:"white", border:"2px solid #EEEDFE", borderRadius:12,
              padding:"8px 16px", fontSize:13, fontWeight:700, cursor:"pointer", color:"#888780" },
  tabActive:{ background:"#7F77DD", borderColor:"#534AB7", color:"white" },
  search:   { background:"white", border:"2px solid #EEEDFE", borderRadius:12,
              padding:"8px 14px", fontSize:13, fontFamily:"'Nunito',sans-serif",
              fontWeight:600, outline:"none", width:220 },
  sideLabel:{ fontSize:10, fontWeight:800, color:"#B4B2A9", letterSpacing:1,
              textTransform:"uppercase", margin:"8px 0 4px 4px" },
  clsItem:  { padding:"8px 12px", borderRadius:10, cursor:"pointer",
              fontSize:13, fontWeight:600, color:"#444441" },
  clsActive:{ background:"#EEEDFE", color:"#3C3489", fontWeight:700 },
  homeCard: { background:"white", borderRadius:16, padding:"20px 24px",
              boxShadow:"0 2px 12px rgba(127,119,221,0.08)", border:"1px solid #EEEDFE" },
  homeCardTitle:{ fontFamily:"'Baloo 2',sans-serif", fontSize:16, fontWeight:800,
                  color:"#3C3489", marginBottom:16 },
  viewAll:  { background:"#EEEDFE", border:"none", borderRadius:12, padding:"10px 20px",
              fontSize:13, fontWeight:700, color:"#534AB7", cursor:"pointer",
              width:"100%", marginTop:12 },
  empty:    { textAlign:"center", padding:32, color:"#B4B2A9", fontWeight:700 },
};
const EC = {
  card:       { background:"white", borderRadius:16, padding:"16px 20px",
                display:"flex", alignItems:"center", gap:16,
                boxShadow:"0 2px 12px rgba(127,119,221,0.08)",
                border:"1px solid #EEEDFE", marginBottom:10 },
  icon:       { fontSize:28, width:48, height:48, background:"#F8F7FF",
  ...
  badgeOpen:  { background:"#E1F5EE", color:"#085041", border:"1px solid #5DCAA5",
                borderRadius:20, padding:"3px 10px", fontSize:11, fontWeight:700 },
  badgeClosed:{ background:"#FCEBEB", color:"#A32D2D", border:"1px solid #F09595",
                borderRadius:20, padding:"3px 10px", fontSize:11, fontWeight:700 },
  btnStart:   { background:"linear-gradient(135deg,#7F77DD,#534AB7)", color:"white",
                border:"none", borderRadius:12, padding:"8px 16px", fontSize:13,
                fontWeight:700, cursor:"pointer", boxShadow:"0 4px 12px rgba(127,119,221,0.3)" },
};
const GR = {
  row:        { display:"flex", alignItems:"center", gap:16, padding:"14px 0",
                borderBottom:"1px solid #EEEDFE" },
  rank:       { width:28, height:28, borderRadius:"50%", background:"#EEEDFE",
  ...
  badge:      { borderRadius:20, padding:"2px 10px", fontSize:11, fontWeight:700,
